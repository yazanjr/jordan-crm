...
function NewDealModal({ onClose, onSubmit }) {
  ...
  const [data, setData] = React.useState({
    ...
    owner: 'Hala Jaber', scope: 'VRF', closeDate: '2026-08-15', probability: 20,
      ...
      <div style={{ padding: '20px 20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        {step === 0 && <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <Field label="Owner">
              <Select value={data.owner} onChange={e => set('owner', e.target.value)} options={[
...
function NotificationsPopover({ anchorRect, onClose, onMarkAllRead }) {
  ...
  const items = [
    { who: 'Rami Haddad', what: 'moved Marriott Amman to Negotiation', when: '5m', icon: Briefcase, unread: true, color: 'var(--img-orange)' },
    { who: 'Sana Khalil', what: 'mentioned you on Mecca Mall expansion', when: '1h', icon: FileI,    unread: true, color: 'var(--img-green)' },
    { who: 'System',      what: 'Royal Jordanian deal idle for 7 days',  when: '3h', icon: Bell,      unread: true, color: 'var(--color-warning)' },
    { who: 'Layla Odeh',  what: 'logged a call with Crowne Plaza Petra',  when: 'yesterday', icon: Phone, unread: false, color: 'var(--neutral-400)' },
    { who: 'Ahmad Marji', what: 'closed Le Meridien Amman — Won 🎉',     when: '2d', icon: Check,     unread: false, color: 'var(--img-green)' },
  ];
  return (
...
function PersonPopover({ anchorRect, onClose, person, setPerson }) {
  const people = ['All people', 'Hala Jaber', 'Rami Haddad', 'Sana Khalil', 'Ahmad Marji', 'Layla Odeh'];
  return (
    <Popover anchorRect={anchorRect} onClose={onClose} width={220}>
      <div style={{ padding: '6px 0' }}>
  ...
  return (
    <Popover anchorRect={anchorRect} onClose={onClose} width={240} align="left">
      <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: 10 }}>
        <window.Avatar name="Hala Jaber" size={36} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 600 }}>Hala Jaber</div>
          <div style={{ fontSize: 11, color: 'var(--fg-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>hala@izzatmarji.com</div>
        </div>
      </div>
      <div style={{ padding: '6px 0' }}>
